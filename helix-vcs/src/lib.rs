...
impl DiffProviderRegistry {
    ...
    pub fn get_diff_base(&self, file: &Path, wst: &WorkspaceTrust) -> Option<Vec<u8>> {
        self.providers
            .iter()
            .find_map(|provider| match provider.get_diff_base(file, wst) {
                Ok(res) => Some(res),
                Err(err) => {
                    log::debug!("{err:#?}");
    ...
    pub fn get_current_head_name(
        ...
        self.providers
            .iter()
            .find_map(|provider| match provider.get_current_head_name(file, wst) {
                Ok(res) => Some(res),
                Err(err) => {
                    log::debug!("{err:#?}");
    ...
    pub fn get_watched_paths(&self, workspace: &Path, wst: &WorkspaceTrust) -> Vec<PathBuf> {
        self.providers
            .iter()
            .filter_map(|provider| provider.get_watched_path(workspace, wst))
            .collect()
    }
    ...
    pub fn for_each_changed_file(
        ...
        tokio::task::spawn_blocking(move || {
            if self
                .providers
                .iter()
                .find_map(|provider| provider.for_each_changed_file(&cwd, &wst, &f).ok())
                .is_none()
            {
...
impl DiffProvider {
    ...
    fn get_diff_base(&self, file: &Path, wst: &WorkspaceTrust) -> Result<Vec<u8>> {
        match self {
            #[cfg(feature = "git")]
            Self::Git => git::get_diff_base(file, wst),
    ...
    fn get_current_head_name(
        ...
        match self {
            #[cfg(feature = "git")]
            Self::Git => git::get_current_head_name(file, wst),
            Self::None => bail!("No diff support compiled in"),
        }
    }
    ...
    fn for_each_changed_file(
        ...
        match self {
            #[cfg(feature = "git")]
            Self::Git => git::for_each_changed_file(cwd, wst, f),
            Self::None => bail!("No diff support compiled in"),
        }
    }

    /// Get the path to watch for VCS state changes (e.g., HEAD file).
    fn get_watched_path(&self, workspace: &Path, wst: &WorkspaceTrust) -> Option<PathBuf> {
        match self {
            #[cfg(feature = "git")]
            Self::Git => git::get_head_path(workspace, wst),
            Self::None => None,
        }
    }
}
