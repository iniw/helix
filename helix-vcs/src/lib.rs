...
impl DiffProvider {
    pub fn needs_reload(&self, fs_event: &helix_core::file_watcher::Event) -> bool {
        match *self {
            #[cfg(feature = "git")]
            DiffProvider::Git(..) => is_git_reload_event(fs_event.path.as_std_path()),
            #[cfg(feature = "jj")]
            DiffProvider::JJ(..) => false,
        }
    }
...
#[cfg(feature = "git")]
fn is_git_reload_event(path: &Path) -> bool {
    if path.ends_with(".git/HEAD") {
        return true;
    }

    if path.file_name().is_some_and(|file| file == "HEAD") {
...
mod tests {
    ...
    fn test_needs_reload_regular_git() {
        ...
        assert!(is_git_reload_event(event.path.as_std_path()));
