...
fn get_next_range(doc: &Rope, start_range: &Range, offs: usize, len_inserted: usize) -> Range {
    let shift = |range: Range| Range::new(range.anchor + offs, range.head + offs);
    ...
    if start_range.head == doc.len_chars() && start_range.anchor == doc.len_chars() {
        return shift(Range::new(start_range.anchor + 1, start_range.head + 1));
    }
    ...
    if len_inserted == 1 {
        ...
        return shift(Range::new(end_anchor, start_range.head + 1));
    }
    ...
    shift(Range::new(end_anchor, end_head))
}
